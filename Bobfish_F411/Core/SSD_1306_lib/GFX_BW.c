...
const tFont* font;
int8_t size = 1;
wchar_t polskie_znaki[]=L"ÓóĄąĆćĘęŁłŃńŚśŹźŻż";
//{'Ó','ó','Ą','ą','Ć','ć','Ę','ę','','ł','Ń','ń','Ś','ś','Ź','ź','Ż','ż'};//
...
int GFX_GetFontCharIndex(wchar_t c){
	if(c>=' '){
		if(c>'~'){
			if(font->length<='~'-' '+1)return -1;
			for(uint8_t i=0;i<18;i++)if(c==polskie_znaki[i])return '~'-' '+1+i;
...
{
	return (*font->image[0]).height;
}

uint8_t GFX_GetFontWidth(wchar_t c)//naprawic(polskie znaki) (chyba OK)
{
	int index=GFX_GetFontCharIndex(c);
	if(index<0)return 0;
	return (*font->image[index]).width;
}

uint8_t GFX_GetFontMinWidth(void){
	uint8_t min=255;
	for(uint8_t i=0;i<font->length;i++){
		if((*font->image[i]).width<min)min=(*font->image[i]).width;
	}
	return min;
}

uint8_t GFX_GetFontMaxWidth(void){
	uint8_t max=0;
	for(uint8_t i=0;i<font->length;i++){
		if((*font->image[i]).width>max)max=(*font->image[i]).width;
	}
	return max;
}

int GFX_GetStringWidth(const wchar_t* str)
{
	int str_width=0;
	while(*str){
		str_width+=GFX_GetFontWidth(*str++);
		str_width+=font->char_space;
	}
	return str_width;
}

void GFX_DrawChar(int x, int y, wchar_t chr, uint8_t color, uint8_t background)//naprawic poskie znanki
{
	if((font->length)<GFX_GetFontCharIndex(chr))return;
	uint8_t chr_width=GFX_GetFontWidth(chr);


	if(chr_width){
		int index=GFX_GetFontCharIndex(chr);
		const tImage* chr_Image=font->image[index];
		uint8_t font_height=GFX_GetFontHeight();
		uint8_t height_bytes=((font_height/8)+((font_height%8) ? 1 : 0));

		for(uint8_t w=0;w<chr_width;w++){

			uint64_t line=0;

			for(uint8_t b=0;b<height_bytes;b++){
				line<<=8;
				line|=((chr_Image->data)[w*height_bytes+b]);
			}
			line=line>>(height_bytes*8-font_height);
			for(uint8_t h=0;h<font_height;h++,line>>=1){
				if(line & 0x01)
				{
					if(size == 1)
						GFX_DrawPixel(x+w, y+h, color);
					else
						GFX_DrawFillRectangle(x+w*size, y+h*size, size, size, color);
				}
				else if(background == 0)
				{
					if(size == 1)
						GFX_DrawPixel(x+w, y+h, background);
					else
						GFX_DrawFillRectangle(x+w*size, y+h*size, size, size, background);
				}
			}
		}
	}
...
void GFX_DrawString(int x, int y, wchar_t *  str, uint8_t color, uint8_t background)
{
	int x_tmp = x;
	wchar_t znak;
	znak = *str;
	while(*str++)
	{
		if((x_tmp+GFX_GetFontWidth(znak))>0&&x_tmp<128) // <- rusuj tylko to co widoczne
		GFX_DrawChar(x_tmp, y, znak, color, background);

		x_tmp += (GFX_GetFontWidth(znak) * size);
		/*if(background == 0)
		{
